...
namespace EmployeeDirectory.Services.Utilities
{
    ...
    internal static class HandleInput
    {
        ...
        {
            int index = SelectIndexFromList(inputFieldName, isRequired, list);
            if(index == -1) { return null; }
            return list[index];
        }

        static int SelectIndexFromList(string inputFieldName, bool isRequired, List<string> list)
        {
            string input;
            int index;
            if (list.Count == 0)
            ...
            while (true)
            {
                ...
                input = input.Trim();
                if (input.Length == 0 && !isRequired) { return -1; }
        ...
        public static int GetRoleId(string inputFieldName, bool isRequired, List<string> list, IRoleServices roleServices, int previousId = 0)
        {
            string? department = SelectFromList(inputFieldName, isRequired, list);
            if(department == null) { return previousId; }
                ...
                while (true)
                {
                    if (int.TryParse(Console.ReadLine(), out int option) && option > 0 && option < 3)
                    {
                        if (option == 1)
                        {
                            ...
                            return ChooseRole("Role", isRequired, roleServices.GetRolesByDepartment(department!), previousId);
                        }
                        else
                        {
        ...
        public static int ChooseRole(string inputFieldName, bool isRequired, List<Role> list, int previousId)
        {
            var roleNames = list.Select(role => role.RoleName).ToList();
            int index = SelectIndexFromList(inputFieldName, isRequired, roleNames);
            if (index == -1) return previousId;
            return list[index].Id;
        }

        public static string? GetManagerId(string inputFieldName, List<Employee> list)
        {
            int index = SelectIndexFromList(inputFieldName, false, list.Select(emp => emp.FirstName + " " + emp.LastName).ToList());
            if (index == -1) return null;
            return list[index].EmpNo;
        }
